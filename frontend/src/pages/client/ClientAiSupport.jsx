...
const CLIENT_PROMPTS = [
  ...
  'Can you suggest suitable lawyers for this case?',
]

export default function ClientAiSupport() {
  const [cases, setCases]       = useState([])
  const [caseId, setCaseId]     = useState('')
  const [messages, setMessages] = useState([])
  const [prompt, setPrompt]     = useState('')
  const [loading, setLoading]   = useState(false)
  const [status, setStatus]     = useState({ text: 'Ready. Select a case and start chatting.', type: 'ok' })
  const boxRef = useRef()
  ...
  const send = async () => {
    ...
    setPrompt('')
    setLoading(true)
    setStatus({ text: 'AI is thinking...', type: 'ok' })
  ...
  return (
    <div>
      <div className="content-header">
        <h1><i className="fas fa-comments mr-3" />AI Support</h1>
        <p>Ask in simple language. I will answer in a conversational way using your selected case details.</p>
      </div>
      ...
      <div className="card mb-4">
        ...
        <div>
          <label className="block text-gray-700 font-semibold mb-2 text-sm">Select Case</label>
      ...
      <div className="card">
        ...
        <div ref={boxRef} className="rounded-xl border border-border bg-gray-50 min-h-[280px] max-h-[420px] overflow-y-auto p-4 mb-4">
          {messages.length === 0 && (
            <p className="text-gray-400 text-sm text-center mt-10">No messages yet. Start the conversation below.</p>
          )}
          {messages.map((m, i) => (
            <div key={i} className={`flex mb-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-xl px-4 py-3 text-sm whitespace-pre-wrap leading-relaxed
                ${m.role === 'user'
                  ...
                  : 'text-gray-900 border border-border bg-white rounded-bl-sm'}
              `}
        ...
        <p className="text-gray-400 text-xs mt-3">
          Tip: You can upload supporting files from <strong>Upload Case</strong>, but it is optional.
        </p>
      </div>
    </div>
  )
